{ config, lib, ... }: {
  output.modules.home.jujutsu = {
    enable = lib.mkEnableOption "jujutsu";

    settings.scopes = lib.mkOption {
      type = with lib.type; listOf attrs;
      default = [];
      example = [
        {
  ...
  config = let jujutsu = config.modules.home.jujutsu;
    in lib.mkIfEnabled jujutsu.enable {
      ...
      programs.jujutsu.settings = {
        ...
        aliases.changelog = lib.mkIfEnabled config.modules.home.nushell.enable
          ["util" "exec" "nu" "~/.config/jj/scripts/changelog.nu"];
